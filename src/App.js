...
class App extends Component {
  ...
  onClickedImage = (id, initialImgId) => {
    console.log(`Big Image: ${initialImgId}`)
    console.log(`clickedImage:${id}`)
    ...
    const newImageId = imagesList[randomNum].id
    this.setState({imgId: newImageId})
    if (id === initialImgId) {
  ...
  onClickResetButton = () => {
    // const {isShown, seconds} = this.state
    this.setState({
      isShown: false,
      imageUrl: imagesList[0].imageUrl,
      imgId: imagesList[0].id,
  ...
  render() {
    const {activeTabId, seconds, imageUrl, imgId, score, isShown} = this.state
          ...
          <div className="nav-bar-content">
            ...
            <ul className="score-timer-container">
              ...
              <li className="timer-container">
                ...
                />
                <p className="score-count">{`${secResult} sec`}</p>
              </li>
            </ul>
          </div>
        </nav>
        ...
        {isShown || seconds === 0 ? (
          <div className="score-card-container">
            <div className="score-card">
              ...
              />
              <p className="result">Your Score</p>
          ...
          <div className="need-container">
            ...
            <ul className="images-list-items">
              {filteredImages.map(eachImage => (
                <ImagesItems
                  imageDetails={eachImage}
                  key={eachImage.imageUrl}
                  topValue={this.state}
